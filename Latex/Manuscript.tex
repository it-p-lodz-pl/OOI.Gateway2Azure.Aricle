...
\begin{figure}
      ...
      \caption{Strategy Domain Model}\label{figure1.StrategyDomainModel}
\end{figure}
...
It is expected that the functionality implementation expressed by these interfaces is provided as independent external composable parts. The composition is accomplished at run time, and the effective application functionality depends essentially on reusable loosely coupled parts composed applying the dependency injection software engineering and adaptive programming concept \todo{cite{RefWorks:doc:5d9796cbe4b0f66c52dccf04}}.
...
\todo{replace RxNetworking by Reactive Application}
...
\begin{figure}
      ...
      \caption{Implementation Architecture}\label{figure3.ImplementationDomainModel}
\end{figure}

\subsection{Architecture}

A generic domain model presenting relationship between the Azure \textit{Cloud-based\ front-end} and \textit{Cyber-physical\ node} attached to the \textit{Cyber-physical\ network} is presented in Fig. \ref*{figure1.StrategyDomainModel}. As it was pointed out in the Sect. \ref*{subs.architecture} using OPC UA PubSub the aggregation of nodes by the network is loosely coupled, i.e. nodes can be added and removed from the network dynamically, and nodes may represent mobile data holding assets.  
...
Implementation of this interface is a basic step to implement \textit{Consumer} functionality. The \textit{DataRepository} represents data holding assets in the \textit{RxNetworking\ App} and, following the proposed architecture, the \textit{IBindingFactory} interface is implemented by this external part. It captures functionality responsible for accessing the process data represented by the \textit{LocalResources}. \todo{duplacted text} The \textit{LocalResources} represents the external part that has a very broad usage purpose. For example, it may be any kind of process data source/destination, and to name a few \textit{Raw\ Data}, \textit{OPC\ UA\ Address\ Space\ Management}, and \textit{Azure} services in this case.

\subsection{Configuration}

Implementation of the \textit{IConfigurationFactory} interface is straightforward and based entirely on the library UAOOI.Configuration.Networking available as the NuGet package. In a typical scenario, this implementation should not be considered for further modification. The only open question is how to provide the name of the file containing the configuration of this role.
...
\begin{figure}
      ...
      \caption{Implementation Architecture}\label{figure5.AzureGateway}
\end{figure}
