...
Many parts in the domain model presented in Fig.~\ref*{figure.UADataApplicationArchitecture} requires a polymorphic approach to implementation. To promote the polymorphic ready solution the following concepts have been adopted:
...
\begin{figure}
      ...
      \caption{Implementation Architecture}\label{figure.ImplementationDomainModel}
\end{figure}

\subsection{Architecture}

A generic domain model presenting relationship between the Azure \textit{Cloud-based\ front-end} and \textit{Cyber-physical\ node} attached to the \textit{Cyber-physical\ network} is presented in Fig. \ref*{figure.StrategyDomainModel}. As it was pointed out in the Sect. \ref*{subs.architecture} using OPC UA PubSub the aggregation of nodes by the network is loosely coupled, i.e. nodes can be added and removed from the network dynamically, and nodes may represent mobile data holding assets.  


\begin{figure}
      \centering
      \includegraphics[width=\textwidth]{../.Media/ImplementationArchitecture.png}
      \caption{Implementation Architecture}\label{figure.ImplementationArchitecture}
\end{figure}

%) \textit{Cloud-based Front-end} and Cyber-physical node, PubSub Gateway, Device, Design and development tools
...
The class \textit{DataManagementSetup} constructor initializes all properties, which are injection points of all parts composing this role.

In this example, it is assumed that \textit{ServiceLocator} \footnote{ \url{https://www.nuget.org/packages/CommonServiceLocator} } is implemented to resolve references to any external services.
...
The Azure supports the HTTP, AMQP, MQTT (Sec. \ref*{azure-main-technology-features}). On other hand the PubSub supports Ethernet, UDP, AMQP, MQTT {Sect. \ref*{subs.architecture}}. If the Ethernet or UDP has been selected to build interconnection based on PubSub direc interoperability between the Azure and PubSub is impossible because the mentioned protocols are not offered as the native communication services.

\subsection{data mapping}

\begin{figure}
      \centering
      \includegraphics[width=\textwidth]{../.Media/AzureGateway.png}
      \caption{Implementation Architecture}\label{figure.AzureGateway}
\end{figure}
